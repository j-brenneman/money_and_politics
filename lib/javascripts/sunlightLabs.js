    ...
    .end(function (response) {
      ...
      if(response.body[0]) {
        ...
        .end(function (response2) {
          ...
          if(response2.body.metadata.bio) {
            var bio = response2.body.metadata.bio
                      .replace(/\r/g, '')
                      .replace(/\n/g, '')
                      .replace(/<p>/g, '')
                      .replace(/<\/p>/g, '');
          } else {
            var bio = 'Information not Available'
          }
          if(response2.body.totals[year])
            amount(response2.body.totals[year].recipient_amount, rep, percent, response2.body.metadata.photo_url, bio);
          else
            amount('Information not Available', rep, percent, response2.body.metadata.photo_url, bio);
        });
      }
      else
